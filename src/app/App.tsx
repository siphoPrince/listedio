...
type View = 'home' | 'upload' | 'detail' | 'checkout' | 'map' | 'dashboard' | 'auth'
  ...
  return (
    <AuthProvider>
      <div className="min-h-screen bg-background">
        ...
        {currentView === 'home' && (
          <Homepage items={mockItems} onItemClick={handleItemClick} searchQuery={searchQuery} />
        )}

        {currentView === 'upload' && <ItemUpload onSuccess={handleUploadSuccess} />}

        {currentView === 'detail' && selectedItem && (
          <ItemDetail item={selectedItem} onBack={handleBackToHome} onCheckout={handleCheckout} />
        )}
        ...
        {currentView === 'map' && <MapView items={mockItems} onItemClick={handleItemClick} />}

        {currentView === 'dashboard' && <UserDashboard onItemClick={handleItemClick} />}

        {currentView === 'auth' && <AuthPage onDone={() => setCurrentView('dashboard')} />}
      </div>
    </AuthProvider>
  )
}
